...
class TestScaleXFromPointInterface:
    ...
    @retry(stop_max_attempt_number=15, wait_fixed=randint(10, 3000))
    def test_get_scale_x_from_points(self) -> None:
        interface: _TestInterface = _TestInterface()
        scale_x: ap.Number = interface.get_scale_x_from_points(
            x=100, y=200)
        assert scale_x == 1.0
        ...
        interface._scale_x_from_points[key_exp_str.value] = ap.Number(0.5)
        scale_x = interface.get_scale_x_from_points(x=100, y=200)
        assert scale_x == 0.5
        assert isinstance(scale_x, ap.Number)
        ...
        interface._scale_x_from_points[key_exp_str.value] = ap.Number(0.3)
        scale_x = interface.get_scale_x_from_points(x=x, y=y)
        assert scale_x == 0.3
        assert isinstance(scale_x, ap.Number)
