...
MAPPING: Dict[str, str] = {
    ...
    'The `animation_rotation_around_point` method interface creates an `ap.AnimationRotationAroundPoint` instance. You can animate rotation around the given point of the instance with it.':  # noqa
    '`animation_rotation_around_point`メソッドのインターフェイスは`ap.AnimationRotationAroundPoint`クラスのインスタンスを生成します。そのインスタンスを使って任意の座標を基準とした回転のアニメーションを設定することができます。',  # noqa
    ...
    'The `animation_rotation_around_point` method requires rotation and x and y coordinates arguments to animate.':  # noqa
    '`animation_rotation_around_point`メソッドは回転角度のrotation、回転の基準座標となるxとyの各引数を必要とします。',  # noqa
    ...
    '**[Interface summary]** Set the rotation around the given point animation setting.<hr>':  # noqa
