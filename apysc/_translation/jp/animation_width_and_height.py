...
MAPPING: Dict[str, str] = {
    ...
    'The `animation_width` method interface creates an `AnimationWidth` instance. You can animate object width with it.':  # noqa
    '`animation_width`インターフェイスは`AnimationWidth`クラスのインスタンスを生成します。そのインスタンスを使って幅のアニメーションを設定することができます。',  # noqa

    'Similarly, the `animation_height` method interface creates an `AnimationHeight` instance. You can animate object height with it.':  # noqa
    '同様に`animation_height`メソッドのインターフェイスは`AnimationHeight`クラスのインスタンスを生成します。そのインスタンスを使って高さのアニメーションを設定することができます。',  # noqa
    ...
    '楕円のインスタンス（`Ellipse`クラス）の`animation_width`と`animation_height`の各インターフェイスは内部実行が異なる都合で`AnimationWidth`クラスなどの代わりに`以下のコード例のようにAnimationWidthForEllipse`クラスと`AnimationHeightForEllipse`クラスのインスタンスを返却します:',  # noqa
