...
def _validate_bezier_3d_continual_pre_data(
    *, path_data_list: List[PathDataBase]
    ...
    for i, path_data in enumerate(path_data_list):
        ...
        if not isinstance(pre_data, PathBezier3D) and not isinstance(
            pre_data, PathBezier3DContinual
        ):
            raise ValueError(
    ...
    *, path_data_list: List[PathDataBase]
    ...
    for i, path_data in enumerate(path_data_list):
        ...
        if not isinstance(path_data, PathBezier2DContinual):
            continue
        pre_data: PathDataBase = path_data_list[i - 1]
        if not isinstance(pre_data, PathBezier2D) and not isinstance(
            pre_data, PathBezier2DContinual
        ):
            raise ValueError(
