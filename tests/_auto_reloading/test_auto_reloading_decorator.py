...
def test__are_files_updated() -> None:
    test_root_dir_path: str = os.path.join("tmp/test_auto_reloading_decorator_1/")
    ...
    last_executed_time = now + timedelta(minutes=-3)
...
def test_set_auto_reloading() -> None:
    global _count
    _count = 0
    test_root_dir_path: str = os.path.join("tmp/test_auto_reloading_decorator_2/")
    shutil.rmtree(test_root_dir_path, ignore_errors=True)
