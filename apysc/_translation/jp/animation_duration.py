...
MAPPING: Dict[str, str] = {
    ...
    ##################################################
    "Each animation method interface (such as the `animation_move`, `animation_x`, and so on) has the `duration`'s argument.": "`animation_move`などの各アニメーションのインターフェイスはこの`duration`引数を持っています。",  # noqa
    ##################################################
    "## Basic usage": "## 基本的な使い方",
    ##################################################
    "The following example sets 3 seconds to the `duration` option and animates an x-coordinate.": "以下の例ではX座標のアニメーションで`duration`の設定に3秒を設定しています。",  # noqa
    ##################################################
