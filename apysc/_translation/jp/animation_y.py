...
MAPPING: Dict[str, str] = {
    ...
    'The `animation_y` method interface creates an `AnimationY` instance. You can animate y-coordinate with it.':  # noqa
    '`animation_y`メソッドのインターフェイスは`AnimationY`クラスのインスタンスを生成します。このインスタンスを使ってY座標のアニメーションを設定することができます。',  # noqa
    ...
    '内部の実装が異なる都合で`Circle`と`Ellipse`の各クラスの`animation_y`のインターフェイスは`AnimationY`クラスの代わりに`AnimationCy`クラスのインスタンスを返却します。',  # noqa
