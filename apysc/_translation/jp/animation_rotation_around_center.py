...
MAPPING: Dict[str, str] = {
    ...
    'This page explains the `animation_rotation_around_center` method interface.':  # noqa
    'このページでは`animation_rotation_around_center`メソッドのインターフェイスについて説明します。',

    '## What interface is this?':
    '## インターフェイス概要',

    'The `animation_rotation_around_center` method interface creates an `ap.AnimationRotationAroundCenter` instance. You can animate rotation around the instance\'s center point with it.':  # noqa
    '`animation_rotation_around_center`メソッドのインターフェイスは`ap.AnimationRotationAroundCenter`クラスのインスタンスを生成します。そのインスタンスを使用して対象のインスタンスの中央座標で回転のアニメーションを設定することができます。',  # noqa
    ...
    '**[Interface summary]** Set the rotation around the center animation setting.<hr>':  # noqa
