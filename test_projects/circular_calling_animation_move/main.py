...
def main() -> None:
    ...
    animation_move.animation_complete(on_animation_complete_1, options=options)
    animation_move.start()

    ap.save_overall_html(dest_dir_path=_DEST_DIR_PATH, minify=False)


def on_animation_complete_1(e: ap.AnimationEvent, options: _RectOptions) -> None:
    """
    ...
    animation_move.animation_complete(on_animation_complete_2, options=options)
    animation_move.start()


def on_animation_complete_2(e: ap.AnimationEvent, options: _RectOptions) -> None:
    """
    The handler will be called when the animation is completed.
