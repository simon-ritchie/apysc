...
def main() -> None:
    ...
    array_1.append(4)
    ap.assert_arrays_equal(left=[1, 2, 3, 4], right=array_1)

    array_1.push(5)
    ap.assert_arrays_equal(left=[1, 2, 3, 4, 5], right=array_1)

    array_2: ap.Array = ap.Array([1, 2])
    array_2.extend([3, 4])
    ap.assert_arrays_equal(left=[1, 2, 3, 4], right=array_2)
    array_3: ap.Array = ap.Array([5, 6])
    array_2.extend(array_3)
    ap.assert_arrays_equal(left=[1, 2, 3, 4, 5, 6], right=array_2)

    array_4: ap.Array = ap.Array([1, 2])
    array_5: ap.Array = array_4.concat([3, 4])
    ap.assert_arrays_equal(left=[1, 2], right=array_4)
    ap.assert_arrays_equal(left=[1, 2, 3, 4], right=array_5)
    array_6: ap.Array = ap.Array([5, 6])
    array_7: ap.Array = array_4.concat(array_6)
    ap.assert_arrays_equal(left=[1, 2, 5, 6], right=array_7)

    array_8: ap.Array[Any] = ap.Array([1, 5])
    array_8.insert(index=1, value=2)
    ap.assert_arrays_equal(left=[1, 2, 5], right=array_8)
    array_8.insert(index=2, value=ap.Int(3))
    ap.assert_arrays_equal(left=[1, 2, 3, 5], right=array_8)
    array_8.insert_at(index=3, value=4)
    ap.assert_arrays_equal(left=[1, 2, 3, 4, 5], right=array_8)

    int_1: ap.Int = ap.Int(2)
    array_9: ap.Array[Any] = ap.Array([1, int_1, 3])
    array_9.pop()
    ap.assert_arrays_equal(left=[1, 2], right=array_9)
    int_2: ap.Int = array_9.pop()
    ap.assert_arrays_equal(left=[1], right=array_9)
    ap.assert_equal(left=2, right=int_2)

    array_10: ap.Array[Any] = ap.Array([1, 2, 3, 4])
    array_10.remove(3)
    ap.assert_arrays_equal(left=[1, 2, 4], right=array_10)
    array_10.remove(ap.Int(2))
    ap.assert_arrays_equal(left=[1, 4], right=array_10)

    array_11: ap.Array = ap.Array([1, 2, 3, 4])
    array_11.remove_at(1)
    ap.assert_arrays_equal(left=[1, 3, 4], right=array_11)
    array_11.remove_at(ap.Int(1))
    ap.assert_arrays_equal(left=[1, 4], right=array_11)

    array_12: ap.Array = ap.Array([1, 2, 3])
    array_12.reverse()
    ap.assert_arrays_equal(left=[3, 2, 1], right=array_12)

    array_13: ap.Array = ap.Array([1, 4, 2, 3])
    array_13.sort()
    ap.assert_arrays_equal(left=[1, 2, 3, 4], right=array_13)

    array_14: ap.Array = ap.Array([1, 4, 2, 3])
    array_14.sort(ascending=False)
    ap.assert_arrays_equal(left=[4, 3, 2, 1], right=array_14)

    array_15: ap.Array = ap.Array([1, 2, 3, 4])
    array_16: ap.Array = array_15.slice(start=1, end=3)
    ap.assert_arrays_equal(left=[2, 3], right=array_16)
    array_17: ap.Array = array_15.slice(start=1)
    ap.assert_arrays_equal(left=[2, 3, 4], right=array_17)
    array_18: ap.Array = array_15.slice(end=2)
    ap.assert_arrays_equal(left=[1, 2], right=array_18)

    array_19: ap.Array = ap.Array([ap.Int(1), ap.Int(2), ap.Int(3)])
    ap.assert_equal(left=2, right=array_19[1])

    array_19[ap.Int(1)] = ap.Int(4)
    ap.assert_arrays_equal(left=[1, 4, 3], right=array_19)

    array_20: ap.Array = ap.Array([1, 2, 3])
    array_20[1] = 4
    ap.assert_arrays_equal(left=[1, 4, 3], right=array_20)

    array_21: ap.Array = ap.Array([1, 2, 3])
    del array_21[1]
    ap.assert_arrays_equal(left=[1, 3], right=array_21)
    ...
    ap.assert_equal(left=1, right=array_24.index_of(int_3))
    ap.assert_equal(left=2, right=array_24.index_of(3))
    ...
    with ap.If(array_25 == array_26, locals_=locals(), globals_=globals()):
        int_4.value = 20
    ap.assert_equal(left=20, right=int_4)
