"""A test project for the draw_rect interface.
...
def main() -> None:
    ...
    rectangle = sprite.graphics.draw_rect(x=700, y=50, width=50, height=50)
    ...
    rectangle = sprite.graphics.draw_rect(x=250, y=150, width=50, height=50)
    rectangle.line_alpha = ap.Number(1.0)

    _another_func(stage=stage, sprite=sprite)

    sprite.graphics.draw_rect(x=450, y=150, width=50, height=50)

    # Test for rectangle fill alpha update with Number.
    number_1: ap.Number = ap.Number(0.725)
    rectangle = sprite.graphics.draw_rect(x=550, y=150, width=50, height=50)
    ...
    line_thickness += 1
    ap.assert_not_equal(left=line_thickness, right=rectangle.line_thickness)
