...
def test_save_plain_txt() -> None:
    ...
    file_util.save_plain_txt(
        txt="To be, or not to be, that is the question.", file_path=tmp_file_path
    )
    txt: str = file_util.read_txt(file_path=tmp_file_path)
    assert txt == "To be, or not to be, that is the question."
    os.remove(tmp_file_path)
...
def test_delete_file_if_exists() -> None:
    ...
    is_deleted: bool = file_util.delete_file_if_exists(file_path=test_file_path)
    assert not is_deleted

    file_util.save_plain_txt(txt="", file_path=test_file_path)
    is_deleted = file_util.delete_file_if_exists(file_path=test_file_path)
    assert is_deleted
...
def test__has_excluding_prefix() -> None:
    ...
    result = file_util._has_excluding_prefix(
        file_name="jp_sprite.md", excluding_file_names_prefix_list=["test_prefix"]
    )
    assert not result
