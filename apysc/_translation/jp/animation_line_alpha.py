...
MAPPING: Dict[str, str] = {
    ...
    'The `animation_line_alpha` method interface creates an `ap.AnimationLineAlpha` instance. You can animate line alpha (opacity) with it.':  # noqa
    '`animation_line_alpha`メソッドのインターフェイスは`ap.AnimationLineAlpha`クラスのインスタンスを生成します。このインスタンスを使って線の透明度のアニメーションを設定することができます。',  # noqa
    ...
    'The following example sets the line alpha animation (from 1.0 to 0.0) with the `animation_line_alpha` method:':  # noqa
