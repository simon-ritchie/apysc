...
class TestPathControlY2MixIn:
    ...
    def test_control_y2(self) -> None:
        ...
        mixin._control_y2 = ap.Number(0)
        mixin.control_y2 = ap.Number(10)
        assert mixin.control_y2 == 10

    @apply_test_settings()
    def test__make_snapshot(self) -> None:
        mixin: PathControlY2MixIn = PathControlY2MixIn()
        mixin._control_y2 = ap.Number(10)
        snapshot_name: str = mixin._get_next_snapshot_name()
        mixin._run_all_make_snapshot_methods(snapshot_name=snapshot_name)
        if mixin._control_y2_snapshots is None:
    ...
    def test__revert(self) -> None:
        ...
        mixin._run_all_make_snapshot_methods(snapshot_name=snapshot_name)
        mixin._control_y2 = ap.Number(20)
        mixin._run_all_revert_methods(snapshot_name=snapshot_name)
    ...
    def test__append_control_y2_linking_setting(self) -> None:
        mixin: PathControlY2MixIn = PathControlY2MixIn()
        mixin._initialize_control_y2_if_not_initialized()
        assert mixin._attr_linking_stack["control_y2"] == [ap.Number(0)]
