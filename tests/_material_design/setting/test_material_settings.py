...
class TestMaterialSettings:
    @apply_test_settings()
    def test_get_color_scheme(self) -> None:
        assert ap.MaterialSettings.get_color_scheme() is None
        ...
        ap.MaterialSettings.set_color_scheme(color_scheme=color_scheme)
        assert ap.MaterialSettings.get_color_scheme() == color_scheme
        ap.MaterialSettings.set_color_scheme(color_scheme=None)

    @apply_test_settings()
    def test_set_color_scheme(self) -> None:
        color_scheme: ap.MaterialColorScheme = ap.MaterialColorScheme(
            primary=ap.Colors.ACID_GREEN_B0BF1A,
            on_primary=ap.Colors.ALGAE_GREEN_64E986,
            secondary=ap.Colors.ALICE_BLUE_F0F8FF,
