...
def test_remove_first_selector_symbol_char() -> None:
    ...
    str_val_2: str = html_util.remove_first_selector_symbol_char(str_val="#line-graph")
    assert str_val_2 == "line-graph"
    assert isinstance(str_val_2, str)

    str_val_3: str = html_util.remove_first_selector_symbol_char(str_val="line-graph")
...
def test_append_html_to_str() -> None:
    result: str = html_util.append_html_to_str(
        to_append_html="<body>", dest_html="<html>", indent_num=1
    )
...
def test_is_script_start_tag_line() -> None:
    result: bool = html_util.is_script_start_tag_line(line="<html>")
    assert not result
    ...
    result = html_util.is_script_start_tag_line(line='<script type="text/javascript">')
    assert result
...
def test_is_script_end_tag_line() -> None:
    ...
    result = html_util.is_script_end_tag_line(
        line='<script src="jquery-3.6.3.min.js"></script>'
    )
    assert not result
...
class TestScriptLineUtil:
    ...
    def test__set_script_line_ranges(self) -> None:
        script_line_util: ScriptLineUtil = ScriptLineUtil(html=self._TEST_HTML)
    ...
    def test_is_script_line(self) -> None:
        ...
        for line_number, expected in expected_values.items():
            result: bool = script_line_util.is_script_line(line_number=line_number)
