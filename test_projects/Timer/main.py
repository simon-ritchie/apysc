"""A test project for the Timer class.
...
def main() -> None:
    ...
    timer_2: ap.Timer = ap.Timer(
        handler=on_timer_2, delay=16.6, repeat_count=100, options=options_1
    )
    timer_2.start()
    ...
    timer_4: ap.Timer = ap.Timer(
        handler=on_timer_2, delay=ap.FPS.FPS_60, repeat_count=100, options=options_1
    )
    ...
    timer_5.timer_complete(handler=on_timer_complete_2, options=options_3)
    timer_5.start()
...
def on_rectangle_click(e: ap.MouseEvent[ap.Rectangle], options: dict) -> None:
    ...
    timer: ap.Timer = ap.Timer(handler=on_timer_1, delay=33.3, options=options_)
    timer.start()
