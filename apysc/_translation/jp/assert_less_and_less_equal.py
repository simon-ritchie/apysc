...
MAPPING: Dict[str, str] = {
    ...
    ##################################################
    "These arguments only accept numeric values, such as the Python built-in `int`, `float`, apysc `Int`, or `Number` value.": "これらの値はPythonビルトインの`int`や`float`、apyscの`Int`や`Number`などの数値の値のみを受け付けます。",  # noqa
    ##################################################
    "The `msg` argument is optional.": "`msg`引数は省略可です。",
    ##################################################
    "This interface displays a specified `msg` (message) argument to the browser console when an assertion fails.": "もしアサーションが失敗した場合には指定された`msg`",  # noqa
    ##################################################
    ...
    ##################################################
    "The following example fails an assertion and displays the `Assertion failed` message on the browser console:": "以下の例ではアサーションは失敗し、`Assertion failed`というメッセージがブラウザのコンソール上に表示されます:",  # noqa
    ##################################################
