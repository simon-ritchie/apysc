...
MAPPING: Dict[str, str] = {
    ...
    ##################################################
    "The `COLORLESS` constant is a no-color setting.": "`COLORLESS`定数は無色の指定用の設定です。",
    ##################################################
    ...
    ##################################################
    "The `apysc` displays no color since the `fill_color` and `line_color` arguments\' values are the `COLORLESS` constant in the following example:": "以下の例では`fill_color`と`line_color`引数の各値に`COLORLESS`定数が指定されているため`apysc`ライブラリでは色を表示しません:",  # noqa
    ##################################################
