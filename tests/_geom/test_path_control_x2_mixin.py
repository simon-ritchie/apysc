...
class TestPathControlX2MixIn:
    ...
    def test_control_x2(self) -> None:
        ...
        mixin._control_x2 = ap.Number(0)
        mixin.control_x2 = ap.Number(10)
        assert mixin.control_x2 == 10

    @apply_test_settings()
    def test__make_snapshot(self) -> None:
        mixin: PathControlX2MixIn = PathControlX2MixIn()
        mixin._control_x2 = ap.Number(10)
        snapshot_name: str = mixin._get_next_snapshot_name()
        mixin._run_all_make_snapshot_methods(snapshot_name=snapshot_name)
        if mixin._control_x2_snapshots is None:
    ...
    def test__revert(self) -> None:
        ...
        mixin._run_all_make_snapshot_methods(snapshot_name=snapshot_name)
        mixin._control_x2 = ap.Number(20)
        mixin._run_all_revert_methods(snapshot_name=snapshot_name)
        assert mixin.control_x2 == 10
    ...
    def test__append_control_x2_linking_setting(self) -> None:
        mixin: PathControlX2MixIn = PathControlX2MixIn()
        mixin._initialize_control_x2_if_not_initialized()
        assert mixin._attr_linking_stack["control_x2"] == [ap.Number(0)]
