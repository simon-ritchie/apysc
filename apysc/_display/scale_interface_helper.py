...
def get_coordinate_key_for_expression(
    *, coordinate: Union[float, Number]
    ...
    if isinstance(coordinate, VariableNameMixIn):
        ...
        coordinate_str = "{:.3f}".format(coordinate)
    key_exp_str: ExpressionString = ExpressionString(value=f"String({coordinate_str})")
    return key_exp_str
...
def get_scale_updating_expression(
    *,
    ...
    before_value_name: str = expression_variables_util.get_next_variable_name(
        type_name=var_names.NUMBER
    )
    key_exp_str_1: ExpressionString = (
        scale_interface_helper.get_coordinate_key_for_expression(coordinate=coordinate)
    )
    ...
    scale_from_point_value_str: str = value_util.get_value_str_for_expression(
        value=scale_dict
    )
