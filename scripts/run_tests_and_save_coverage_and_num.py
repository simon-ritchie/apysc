...
def _save_passing_tests_num(stdout: str) -> None:
    ...
    for line in lines:
        if " passed in " not in line:
            continue
        match: Optional[Match] = re.search(pattern=r".*? (\d+?) passed", string=line)
        if match is None:
            continue
        passing_test_num = match.group(1)
    logger.info(
        "Saving a passing tests number to the .env file : " f"{passing_test_num}"
    )
...
def _save_coverage(stdout: str) -> None:
    ...
    for line in lines:
        ...
        match: Optional[Match] = re.search(
            pattern=r"TOTAL\s+?(\d+?)\s+?(\d+?)\s", string=line
        )
        if match is None:
    ...
    logger.info("Saving a coverage to the .env file: " f"{coverage}")
    with open(".env", "a") as f:
